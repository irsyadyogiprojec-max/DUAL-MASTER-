...
st.set_page_config(page_title="Input Part Repair", page_icon="🛠️", layout="centered")
...
if "repair_type" not in st.session_state:
    st.session_state["repair_type"] = ""
if "repair_sn" not in st.session_state:
...
def scan_label_api(pil_img):
    try:
        ...
        if res.get("ParsedResults"):
            ...
            type_m = re.search(r'(DME-[0-9]+)', text)
            d_type = type_m.group(1) if type_m else "DME-010"
            
            # Deteksi SERIAL No. (Contoh: 2CB0421 A)
            sn_m = re.search(r'([0-9][A-Z0-9]{5,8}\s*[A-Z1L])', text)
            d_sn = sn_m.group(1) if sn_m else "2CB0421 A"
            
            # Auto-Koreksi huruf L / 1 menjadi A di akhir Serial No.
            d_sn = re.sub(r'(\d[A-Z0-9]{5,7}\s+)[L1I]$', r'\1A', d_sn)
...
# 4. Styling Tampilan Dark Theme (Sama Persis Seperti Part NG)
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700&display=swap');
    html, body, [class*="css"] { font-family: 'Plus Jakarta Sans', sans-serif; }
    .stApp { background: linear-gradient(135deg, #0F1015 0%, #171922 100%); color: #E2E8F0; }
    .block-container { max-width: 800px !important; padding-top: 2rem; padding-bottom: 3rem; }
    .title-text { color: #F3E5AB; font-size: 26px; font-weight: 700; margin-bottom: 4px; }
    .subtitle-text { color: #94A3B8; font-size: 14px; margin-bottom: 20px; }
    label { color: #CBD5E1 !important; font-weight: 500 !important; font-size: 14px !important; }
...
# 6. Header
st.markdown('<div class="title-text">🛠️ Input Part Repair</div>', unsafe_allow_html=True)
st.markdown('<div class="subtitle-text">Formulir pencatatan dan proses perbaikan (repair) part</div>', unsafe_allow_html=True)
...
if uploaded_file is not None:
    ...
    with st.spinner("🔍 Memindai TYPE & SERIAL No..."):
        ...
        st.session_state["repair_type"] = d_type
        st.session_state["repair_sn"] = d_sn

st.write("")

# 8. Form Utama
with st.form("form_repair", clear_on_submit=False):
    ...
    with col2:
        ...
        type_part = st.text_input("TYPE", value=st.session_state.get("repair_type", ""), placeholder="Contoh: DME-010")
        no_seri = st.text_input("SERIAL No. Part", value=st.session_state.get("repair_sn", ""), placeholder="Contoh: 2CB0421 A")

    submitted = st.form_submit_button("🛠️ Proses & Simpan Data Repair")
    ...
    if submitted:
        if teknisi == "-- Pilih MP --":
            ...
            with st.spinner("⏳ Mengirim data perbaikan ke Google Spreadsheet..."):
                try:
                    res = requests.post(SHEETS_URL, json=payload, timeout=10)
                    st.success(f"Berhasil! Data perbaikan tersimpan di Spreadsheet (Line: {line})")
                    st.balloons()
